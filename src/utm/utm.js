...
const utm = {
  ...
  toLatLon: (easting, northing, zoneNum, zoneLetter, northern, strict) => {
    ...
    if (zoneLetter) {
      ...
      if (
        zoneLetter.length !== 1 ||
        utm.ZONE_LETTERS.indexOf(zoneLetter) === -1
      ) {
        throw new RangeError(
    ...
    const latitude =
      ...
      (pTan / r) *
        (d2 / 2 -
          (d4 / 24) * (5 + 3 * pTan2 + 10 * c - 4 * c2 - 9 * utm.E_P2)) +
      (d6 / 720) *
    ...
    const longitude =
      (d -
        (d3 / 6) * (1 + 2 * pTan2 + c) +
        (d5 / 120) *
          (5 - 2 * c + 28 * pTan2 - 3 * c2 + 8 * utm.E_P2 + 24 * pTan4)) /
      pCos;

    return {
      latitude: utm.toDegrees(latitude),
      longitude:
        utm.toDegrees(longitude) + utm.zoneNumberToCentralLongitude(zoneNum),
    };
  },
  ...
  fromLatLon: (latitude, longitude, forceZoneNum) => {
    ...
    const easting =
      utm.K0 *
        ...
        (a +
          (a3 / 6) * (1 - latTan2 + c) +
          (a5 / 120) * (5 - 18 * latTan2 + latTan4 + 72 * c - 58 * utm.E_P2)) +
      500000;
    let northing =
      utm.K0 *
      (m +
        n *
          ...
          (a2 / 2 +
            ...
            (a6 / 720) *
              (61 - 58 * latTan2 + latTan4 + 600 * c - 330 * utm.E_P2)));
